...
test end-to-end. Reuses build_lakehouse.write_chunk with the real lakehouse
config so the fixture can never drift from the curated schema.
...
def main() -> None:
    ...
    bl.write_chunk([make_row(i) for i in range(ROW_COUNT)], output_path, CONFIG["lakehouse"])
    logger.info("Wrote {}-row fixture to {}", ROW_COUNT, output_path)
